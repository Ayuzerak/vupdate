...
def insert_update_service_addon():
    ...
    if not method_defined:
        ...
        new_method = [
            ...
            f"{method_indent}    updated_flag_path = os.path.join(addon_path, \"updatededed\")\n",
    ...
    try:
        ...
        for sample in samples:
            ...
            if original_matches != transformed_matches:
                logger.info(f"Mismatch found for input: {repr(sample)}")
                logger.debug(f"Original: {original_matches}, Transformed: {transformed_matches}")
                return False
        return True
    except re.error as regex_error:
        logger.error(f"Regex error during testing: {regex_error}")
        return False
    except Exception as e:
...
def check_for_regex_in_function_calls(code):
    ...
    """
    logger.info("Checking for regex in function calls...")
    regex_patterns = []
    ...
    logger.info(f"Found regex patterns in function calls: {regex_patterns}")
...
def find_regex_in_ast(tree):
    ...
    """
    logger.info("Searching for regex patterns in AST...")
    regex_patterns = []
    # For Python 3.8+, string constants are stored in ast.Constant nodes.
                ...
                if isinstance(target, ast.Name):
                    if 'pattern' in target.id.lower() or 'regex' in target.id.lower():
                        if isinstance(node.value, (ast.Str, ast.Constant)):
                            # Use node.value.s for ast.Str or node.value.value for ast.Constant.
    ...
    logger.info(f"Found regex patterns in AST: {regex_patterns}")
    return regex_patterns
            ...
            if any(token in node.value for token in ['\\', '.', '*', '+', '?', '^', '$', '[', ']', '(', ')']):
                if is_valid_regex(node.value):
                    ...
                    if safe_pattern != node.value:
                        logger.info(f"Transforming regex: {node.value} -> {safe_pattern}")
                        return ast.copy_location(ast.Constant(value=safe_pattern), node)
        return node

    # For backward compatibility with Python versions using ast.Str
    def visit_Str(self, node):
        if is_valid_regex(node.s):
            safe_pattern = safe_regex_pattern(node.s)
            if safe_pattern != node.s:
                logger.info(f"Transforming regex: {node.s} -> {safe_pattern}")
...
def rewrite_file_to_avoid_regex_infinite_loops(file_path):
    ...
    try:
        ...
        # Transform the AST to rewrite regex patterns
        ...
        try:
            ...
            # For Python versions earlier than 3.9, fallback to astor:
            import astor
        ...
        if new_code != file_contents:
            with open(file_path, 'w', encoding='utf-8') as file:
                file.write(new_code)
            logger.info("File rewritten to avoid regex infinite loops and inefficiencies.")
        else:
        ...
        logger.error(f"Unexpected error while modifying file: {e}")
